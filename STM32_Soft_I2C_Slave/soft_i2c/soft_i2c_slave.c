...
#define SOFT_I2C_SLAVE_SDA_PIN_NUMBER  6
...
static void Soft_I2C_Slave_SDA_IN_INT()
    {
    SET_BIT(SOFT_I2C_SLAVE_SDA_PORT->AFR[0], (0x00 << SOFT_I2C_SLAVE_SDA_PIN_NUMBER));
    MODIFY_REG(SOFT_I2C_SLAVE_SDA_PORT->MODER, \
	      (0x03 << SOFT_I2C_SLAVE_SDA_PIN_NUMBER*2), \
	      (0x02 << SOFT_I2C_SLAVE_SDA_PIN_NUMBER*2));
    }
static void Soft_I2C_Slave_SDA_OUT_OD()
    {
    }
...
static void Soft_I2C_Slave_State_Machine_SDA_ISR()
	...
	if (!Soft_I2C_Slave_SDA_Read())
	    ...
	    Soft_I2C_Slave.Bit_Count = 0;
	    Soft_I2C_Slave.Current_Byte = 0;
	    Soft_I2C_Slave.Received_Address = 0;
	    Soft_I2C_Slave.Buffer_Index = 0;
	    Soft_I2C_Slave.Byte_Count = 0;
	    Soft_I2C_Slave_SCL_Disable_INT();
	    }
...
static void Soft_I2C_Slave_State_Machine_SCL_ISR()
	...
	if (Soft_I2C_Slave.state == Slave_Detect_Start
	    ...
	    {
	    Soft_I2C_Slave.Received_Address <<= 1;
	    if (Soft_I2C_Slave_SDA_Read())
		{
		Soft_I2C_Slave.Received_Address |= 0x01;
		}
	    Soft_I2C_Slave.Bit_Count++;
	    if (Soft_I2C_Slave.Bit_Count == 8)
		{
		Soft_I2C_Slave.Bit_Count = 0;
		if ((Soft_I2C_Slave.Received_Address & 0xFE)
