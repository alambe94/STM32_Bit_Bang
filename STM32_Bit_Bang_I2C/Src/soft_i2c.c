...
void Soft_I2C_Master_Add(Soft_I2C_Master_t *i2c_handle)
	...
	i2c_handle->Ring_Buffer.Read_Index = 0;
	i2c_handle->Ring_Buffer.Write_Index = 0;
...
void Soft_I2C_Master_Write_Byte(Soft_I2C_Master_t *i2c_handle,
	...
	Ring_Buffer_Put_Char(&i2c_handle->Ring_Buffer, (register_address >> 8));
	}

    Ring_Buffer_Put_Char(&i2c_handle->Ring_Buffer, register_address);

    Ring_Buffer_Put_Char(&i2c_handle->Ring_Buffer, byte);
...
void Soft_I2C_Master_Read_Byte(Soft_I2C_Master_t *i2c_handle,
    ...
    i2c_handle->Address_RW = (slave_address << 1) + 1;

    i2c_handle->Bytes_To_Read = 1;

    Ring_Buffer_Flush(&i2c_handle->Ring_Buffer);
...
void Soft_I2C_Master_Write_Bytes(Soft_I2C_Master_t *i2c_handle,
    ...
    while (len--)
	{
	Ring_Buffer_Put_Char(&i2c_handle->Ring_Buffer, *bytes++);
	}
...
void Soft_I2C_Master_TIM_ISR(TIM_HandleTypeDef *htim)
	...
	if (i2c_handle->Active_Flag)
		...
		if (i2c_handle->State == Generate_Start)
		    {
		    /*nothing to do in Generate_Start when clock is low*/
		    }
		else if (i2c_handle->State == Data_Out)
		    {
